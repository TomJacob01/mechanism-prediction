...
def test_dataset_to_loss_pipeline(sample_reactions, task_mode, num_classes, shift):
    ...
    dataset = MechUSPTODataset(sample_reactions, task_mode=task_mode, compute_spectators=True)
    batch = collate_fn_with_spectators([dataset[i] for i in range(len(dataset))])
    assert batch is not None

    model = ReactionTransformer(hidden_dim=32, num_heads=4, num_layers=2, num_classes=num_classes)
    ...
    loss = criterion(logits, targets, mask_2d, getattr(batch, "spectator_padded", None))
